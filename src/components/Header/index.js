...
const Header = () => {
	...
	return (
		<header className='flex items-center justify-between w-full p-4 px-5 sm:px-10'>
			...
			<nav
				className='fixed z-50 flex items-center px-6 py-3 font-medium capitalize transition-all duration-300 translate-x-1/2 border border-solid rounded-full w-max sm:px-8 border-dark sm:hidden top-6 right-1/2 bg-light dark:bg-darkOffset backdrop-blur-sm ease'
				style={{
					top: click ? '1rem' : '-5rem',
				}}
			>
			...
			<nav className='fixed z-50 items-center hidden px-8 py-3 font-medium capitalize translate-x-1/2 border border-solid rounded-full w-max border-dark sm:flex top-6 right-1/2 bg-light dark:bg-darkOffset backdrop-blur-sm'>
				<Link href='/' className='mr-2'>
					Home
				</Link>
				<Link href='/about' className='mx-2'>
